...
export function commsWestside(state: State | null, pressed: ArrowStr | null, userResources: number, userUnlocks: UserUnlocks[]): [string, State] {
  if (!state)
    state = { cursor: 0, focused: null }

  const items = shopItems[WorldsEntities.MERCHANT_WESTSIDE_OUTPOST]!.filter(i => userMeetsRequirements(i.requires, userUnlocks) && i.unlocks.some(i => !userUnlocks.includes(i)))
  ...
  if (state.focused === null) {
    ...
    if (pressed === 'left') state.focused = null
      ...
      useSocket().sendPurchasePacket(WorldsEntities.MERCHANT_WESTSIDE_OUTPOST, items[state.focused].name)
